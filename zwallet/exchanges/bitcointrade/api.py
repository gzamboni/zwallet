...
TRADE_METHOD = 'trades?start_time={start_time}&end_time={end_time}&page_size={page_size}&current_page=1'  # noqa: E501


class BitcoinTradePublic:
    def __init__(self, version='v3', coin='BRLBTC'):
        self.url = 'https://api.bitcointrade.com.br/{version}/public/{coin}/{method}/'  # noqa: E501
    ...
    def get_orders(self, method='orders', order_by='unit_price'):
        ...
        if orders_dict.get('data'):
            ...
            ordered_orders_list = sorted(
                orders_list,
                key=lambda k: k[order_by]
            )
            response.close()
            return {
                'message': orders_dict.get('message'),
                'data': ordered_orders_list
            }
        response.close()
        return response.json()
    ...
    def get_trades(self,
                   method=TRADE_METHOD,
                   start_time=(datetime.datetime.now() - datetime.timedelta(days=1)).isoformat(),  # noqa: E501
                   end_time=datetime.datetime.now().isoformat(),
                   page_size=100):
        """https://api.bitcointrade.com.br/v1/public/BTC/trades?
        start_time=2016-10-01T00:00:00-03:00&end_time=2018-10-10T23:59:59-03:00&page_size=100&current_page=1"""   # noqa: E501
        response = requests.get(self.url.format(
            version=self.version,
            coin=self.coin,
...
class BitcoinTradeBitcoin:
    def __init__(self, api_token, version='v3', coin='BRLBTC'):
        super().__init__()
    ...
    def get_withdraw_fee(self, method='withdraw/fee'):
        ...
        auth_header = {
            'Authorization': 'ApiToken {api_token}'.format(
    ...
    def get_withdraw_list(self,
                          method='withdraw?page_size={page_size}&current_page=1&'   # noqa: E501
                                 'status={status}&start_date={start_date}&end_date={end_date}',  # noqa: E501
                          page_size=100,
                          status='pending',
                          start_date=(datetime.datetime.now() - datetime.timedelta(days=1)).strftime('%Y-%m-%d'),  # noqa: E501
                          end_date=datetime.datetime.now().strftime('%Y-%m-%d')):  # noqa: E501
        auth_header = {
            'Authorization': 'ApiToken {api_token}'.format(
                api_token=self.api_token
            )
    ...
    def get_deposit_list(self,
                         method='deposits?page_size={page_size}&current_page=1&'  # noqa: E501
                                'status={status}&start_date={start_date}&end_date={end_date}',  # noqa: E501
...
class BitcoinTradeMarket:
    ...
    def get_users_orders(
        ...
        'end_date={end_date}&currency={coin}&type={type}&page_size={page_size}&current_page=1',  # noqa: E501
    ...
    def get_estimated_price(
            self,
            method='/estimated_price?amount={amount}&currency={coin}&type={_type}',  # noqa: E501
            amount=0,
            _type='buy'):
        auth_header = {
            'Authorization': 'ApiToken {api_token}'.format(
